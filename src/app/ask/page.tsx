...
const ASK_PAGE = {
  ...
  description:
    'A direct line to Pilot\'s knowledge manifold. Ask about wisdom engines, knowledge projection, editorial voice, or provenance — and Pilot will answer from its actual source material, citing everything it draws from.',
  placeholder: 'What would you like to know about Pilot?',
}

const ASK_SUGGESTIONS = [
  'What makes Pilot different from a headless CMS?',
  'How does the editorial voice constitution work?',
  'What is knowledge projection?',
  'How does Pilot trace provenance?',
]
...
function AskContent() {
  ...
  const sendMessage = useCallback(
    async (q: string) => {
      ...
      // Graceful degradation: if no token arrives within STREAM_TIMEOUT_MS, show fallback
      const timeoutId = setTimeout(() => {
        setMessages((prev) => {
          const next = [...prev]
          const msg = next[assistantIndex] as AssistantMessage
          if (msg && msg.streaming && msg.items.length === 0) {
            next[assistantIndex] = {
              ...msg,
              streaming: false,
              error:
                'Pilot is taking longer than usual to respond. The manifold may be busy — please try again in a moment.',
            }
          }
          return next
        })
      ...
      try {
        await mcpRef.current!.callToolStreaming(
          ...
          (chunk) => {
            ...
            appendChunk(assistantIndex, chunk)
          }
        )
      } catch (err: any) {
        clearTimeout(timeoutId)
  ...
  return (
    <section className="py-12">
      <div className="max-w-2xl mb-10">
        <Eyebrow text={ASK_PAGE.eyebrow} className="mb-4" />
        <h2 className="font-serif text-[36px] md:text-[44px] leading-tight text-ink mb-4">
          {ASK_PAGE.title}
        </h2>
        <p className="text-slate text-base leading-relaxed">{ASK_PAGE.description}</p>
      </div>
      ...
      <div className="max-w-2xl space-y-4">
        ...
        {!hasMessages && (
          <SuggestionPills suggestions={ASK_SUGGESTIONS} onSelect={sendMessage} />
        )}
      </div>
    </section>
  )
